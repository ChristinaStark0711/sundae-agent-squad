...
.claude/skills/         slash commands: /build-video, /new-video-brief, /new-squad
...
7. **Adding a squad:** run `/new-squad <name>`. It scaffolds `squads/<name>/` and `.claude/agents/<name>-*.md` from `squads/_template/`, then add a row to the table above.

## Tool names

Connected MCP servers appear as `mcp__Open_Art__*`, `mcp__Higgsfield__*` and `mcp__github__*`.
